...
use godot::obj::{Base, GodotClass, WithBaseField};
use godot::prelude::{Export, FromGodot, Gd, load, ToGodot};
...
impl IArea2D for UIButton {
    ...
    fn process(&mut self, _ : f64) {
        ...
        if self.get_hovered() && input.is_action_pressed(StringName::from_str("click").unwrap()) && self.get_interact_timer().is_stopped() && self.get_enabled() {
...
impl UIButton {
    ...
    pub fn get_enabled(&mut self) -> bool {
        self.base().get_parent().unwrap().try_cast::<ButtonContainer>().unwrap().bind_mut().get_enabled()
    }

    #[func]
    pub fn do_button_action(&mut self) {
        self.base_mut().emit_signal("button_pressed".into(), &[]);
    }
    ...
    pub fn get_interact_timer(&mut self) -> Gd<Timer> {
        self.base().get_parent().unwrap().cast::<ButtonContainer>().bind_mut().get_interact_timer()
    }

    fn get_hovered_sprite(&mut self) -> Gd<Sprite2D> {
        self.base().get_node_as::<Sprite2D>("hovered")
    }
